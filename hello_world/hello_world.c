...
hello_world(void)
{
	...
	TAILQ_FOREACH(ns_entry, &g_namespaces, link) {
		...
		// printf("call spdk_nvme_qpair_process_completions, count: %d\n time spent %ld ns ", comp_count,  elapsed_time);
		printf("call spdk_nvme_qpair_process_completions, count: %d\n  ", comp_count);
...
alsp_init(void)
{
	...
	printf("call spdk_nvme_trid_populate_transport\n");

	spdk_nvme_trid_populate_transport(&g_trid, SPDK_NVME_TRANSPORT_PCIE);
	snprintf(g_trid.subnqn, sizeof(g_trid.subnqn), "%s", SPDK_NVMF_DISCOVERY_NQN);

	printf("call spdk_env_init\n");
